...
// ----2. Variables globales para la paginacion
let currentPage = 0;
let currentSize = 10;
 
// Todo el contenido ira aqui
...
form.addEventListener("submit", async(e) =>
{
    ...
    if(file)
    {
        try
        {  
            const data = await uploadImageToFolder(file, "producto");
    ...
    {
        console.log("Error de guardar", err);
    }
});
 ...
 async function cargarProductos() {
    try {
      ...
      items.forEach((item) => {
        ...
        if (item.imagen) {
          ...
          img.className = "thumb";
          img.alt = "img";
          img.src = item.imagen; // Opcional: validar dominio aquí
  ...
  async function cargarCategorias()
  {
    try {
        ...
        otp.selected = true;
        otp.hidden = true;
        otp.textContent = "Seleccione...";
        select.appendChild(otp);
     
        // Insertar categorías
        categories.forEach((c) => {
          const option = document.createElement("option");
    option.value = c.idCategoria ?? c.id ?? c.value;
          option.textContent = c.nombreCategoria ?? c.nombre ?? c.text;
          option.title = c.descripcionCategoria ?? c.descripcion ?? ""; // tooltip
